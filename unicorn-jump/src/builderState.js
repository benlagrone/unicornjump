...
export const FURNITURE_CATALOG = [
  ...
  createFurniture({
    id: 'courtyard-cushion',
    name: 'Courtyard Cushion',
    width: BUILDER_ROOM_GRID_SIZE * 3,
    height: BUILDER_ROOM_GRID_SIZE * 2,
    color: '#ffd7bc',
    accent: '#d8744b',
  ...
  createFurniture({
    id: 'feather-altar-shelf',
    name: 'Feather Altar Shelf',
    width: BUILDER_ROOM_GRID_SIZE * 3,
    height: BUILDER_ROOM_GRID_SIZE * 3,
    color: '#d6b788',
    accent: '#7c532e',
  ...
  createFurniture({
    id: 'lacquer-display-shelf',
    name: 'Lacquer Display Shelf',
    width: BUILDER_ROOM_GRID_SIZE * 3,
    height: BUILDER_ROOM_GRID_SIZE * 3,
    color: '#f3e9e1',
    accent: '#2f405b',
...
const getCatalogSeed = (value = '') =>
  String(value).split('').reduce((seed, char) => ((seed * 33) ^ char.charCodeAt(0)) >>> 0, 5381);

export const getFurnitureCatalogForRoom = (themeId, roomSeed = themeId) => {
  const catalog = getFurnitureCatalogForTheme(themeId);
  if (catalog.length <= 1) {
    return catalog;
  }

  const offset = getCatalogSeed(`${themeId}:${roomSeed}`) % catalog.length;
